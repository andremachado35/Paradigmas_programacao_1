    ...
    Date d1 = cal1.getTime(); out.println(d1);
     ...
     try {
        DataOutputStream dout = new DataOutputStream(
                 new FileOutputStream("fich551.txt"));
        for(Produto pr : stock.values()) {
          dout.writeUTF("---------------------");
          dout.writeUTF(pr.daCod());
          dout.writeUTF(pr.daNome());
          dout.writeInt(pr.daQuant());
          comps = pr.daComps();
          dout.writeInt(comps.size());
          for(String c : comps) dout.writeUTF(c);
        }
